...
data class AgentMetadataPreview(
...
object PlannerLabRunner {
    ...
    suspend fun run(mode: PlannerLabMode, metadata: AgentMetadataPreview, task: String): PlannerLabRunResult = when (mode) {
    ...
    ): AgentMetadataPreview = AgentMetadataPreview(
        modelInfo = ModelInfo(
    ...
    private suspend fun runSimplePlanner(metadata: AgentMetadataPreview, task: String): PlannerLabRunResult {
        ...
        return PlannerLabRunResult(
            ...
            notes = listOf(
                "使用本地 deterministic PromptExecutor，不依赖真实网络或 API key。",
                "ModelInfo 与 HiddenString 仍基于当前 Agent Config 生成，方便 UI 统一展示。",
            ),
    ...
    private suspend fun runGoapPlanner(metadata: AgentMetadataPreview): PlannerLabRunResult {
